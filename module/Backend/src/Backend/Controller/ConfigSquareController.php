            ...
            if (is_numeric($rulesCount) && $rulesCount > 0) {
                ...
                try {
                    ...
                    for ($i = 0; $i < $rulesCount; $i++) {
                        ...
                        $rule = urldecode($rule);
                        $rule = json_decode($rule, true); // Force associative array

                        // Validate rule structure
                        if (!is_array($rule) || count($rule) < 12) {
                            throw new \Exception('Invalid pricing rule structure at index ' . $i);
                        }
                        ...
                        // Add per_quantity field (default to false/0)
                        $rule[12] = 0;
