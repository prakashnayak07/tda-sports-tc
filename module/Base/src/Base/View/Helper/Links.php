            ...
            if ($links && is_array($links) && !empty($links)) {
                ...
                foreach ($links as $title => $href) {
                    if (!empty($title) && !empty($href)) {
                        $html .= sprintf(
                            '<a href="%s" class="inline-flex items-center justify-center px-4 py-2 text-sm font-medium text-white bg-[#93b353] rounded-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition-colors duration-200 min-w-0">',
