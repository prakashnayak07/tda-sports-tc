...
class SquarePricingManager extends AbstractManager
{
    ...
    public function create(array $rules = array())
    {
        ...
        try {
            ...
            $statement = $adapter->query(
                'INSERT INTO ' . SquarePricingTable::NAME . ' (sid, priority, date_start, date_end, day_start, day_end, time_start, time_end, price, rate, gross, per_time_block, per_quantity) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
            );
            ...
            // Update the in-memory cache with the new rules
            $this->rules = array();
            foreach ($rules as $rule) {
                $this->rules[] = $rule;
            }
    ...
    public function getPricingRule($dateTime, $square)
    {
        ...
        // DEBUG: Track rule matching
        $debugDateTime = is_string($dateTime) ? $dateTime : $dateTime->format('Y-m-d H:i:s');
        $debugSquare = ($square instanceof Square) ? $square->get('sid') : $square;
        error_log("DEBUG getPricingRule - Looking for: DateTime=$debugDateTime, Square=$debugSquare");
        error_log("DEBUG getPricingRule - Total rules to check: " . count($this->rules));
        ...
        foreach ($this->rules as $ruleIndex => $rule) {
            error_log("DEBUG - Checking rule $ruleIndex: sid={$rule['sid']}, dates={$rule['date_start']} to {$rule['date_end']}, times={$rule['time_start']} to {$rule['time_end']}, price={$rule['price']}");
            ...
            if ($dateTime <= $dateEnd && $dateTime >= $dateStart) {
                ...
                error_log("DEBUG - Date range matches. DateTime day: $dateTimeDay, Rule days: {$rule['day_start']} to {$rule['day_end']}");
                    ...
                    if ($dateTime >= $ruleDateTime) {
                        ...
                        if ($dateTime < $ruleDateTime) {
                            if (is_null($rule['sid']) || $rule['sid'] == $square) {
                                error_log("DEBUG - RULE MATCHED! Rule $ruleIndex matches all criteria");
                                $matchedRule = $rule;
                            } else {
                                error_log("DEBUG - Rule $ruleIndex rejected: wrong square (rule sid={$rule['sid']}, requested=$square)");
                            }
                        } else {
                            error_log("DEBUG - Rule $ruleIndex rejected: time too late");
                        }
                    } else {
                        error_log("DEBUG - Rule $ruleIndex rejected: time too early");
                    }
                } else {
                    error_log("DEBUG - Rule $ruleIndex rejected: wrong day");
                }
            } else {
                error_log("DEBUG - Rule $ruleIndex rejected: wrong date");
            }
        }

        error_log("DEBUG getPricingRule - Final result: " . ($matchedRule ? "FOUND rule with price {$matchedRule['price']}" : "NO MATCH"));
        return $matchedRule;
    }
    ...
    {
        // DEBUG: Core pricing calculation
        error_log("DEBUG getFinalPricing - Date: " . (is_string($date) ? $date : $date->format('Y-m-d')));
        error_log("DEBUG getFinalPricing - Time: $timeStart to $timeEnd");
        error_log("DEBUG getFinalPricing - Square ID: " . $square->get('sid'));
        error_log("DEBUG getFinalPricing - Available rules: " . count($this->rules));
    ...
    public function getFinalPricingInRange(DateTime $dateTimeStart, DateTime $dateTimeEnd, Square $square, $quantity)
    {
        ...
        error_log("DEBUG SquarePricingManager - getFinalPricingInRange called");
        error_log("DEBUG - Date Start: " . $dateTimeStart->format('Y-m-d H:i:s'));
        error_log("DEBUG - Date End: " . $dateTimeEnd->format('Y-m-d H:i:s'));
        error_log("DEBUG - Square ID: " . $square->get('sid'));
        error_log("DEBUG - Quantity: " . $quantity);
        error_log("DEBUG - Total rules loaded: " . count($this->rules));
